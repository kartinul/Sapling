      ...
      if (message.channel.type !== "dm" && !message.author.bot) {
        for (const alias of commands) {
          ...
          if (
            ...
            try { console.log('⚫ ' + message.member.user.tag + ' used command "' + prefix + commands[0] + '"') } catch { }
            // Ensure the user has the required permissions
            for (const permission of permissions) {
              if (!member.hasPermission(permission)) {
                message.inlineReply(permissionError, {
            ...
            if (disabled) { return message.channel.send("Sorry, cannot use that command right now... because it's disabled! :(") }
